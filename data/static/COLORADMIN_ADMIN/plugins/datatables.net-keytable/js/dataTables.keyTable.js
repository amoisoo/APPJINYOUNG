...
var KeyTable = function ( dt, opts ) {
	...
	this.s = {
		...
		/** @type {Node} Input element for tabbing into the ui_table */
		tabInput: null
	};
	...
	// Check if row reorder has already been initialised on this ui_table
...
$.extend( KeyTable.prototype, {
	...
	/**
	 * Blur the ui_table's cell focus
	 */
	blur: function ()
	{
		this._blur();
	...
	/**
	 * Enable cell focus for the ui_table
	 *
	...
	_constructor: function ()
	{
		...
		// Need to be able to calculate the cell positions relative to the ui_table
		...
		var editInline = function () {
			editor
				...
				.one( 'cancelOpen'+namespace, function () {
					// `preOpen` can cancel the display of the ui_form, so it
	...
	/**
	 * Focus on a particular cell, shifting the ui_table's paging if required
	 *
	...
	_scroll: function ( container, scroller, cell, posOff )
	{
		...
		// If Scroller is being used, the ui_table can be `position: absolute` and that
	...
	/**
	 * Calculate a single offset movement in the ui_table - up, down, left and
	 * right and then perform the focus if possible
	 *
	 * @param  {object}  e           Event object
	 * @param  {string}  direction   Movement direction
	 * @param  {boolean} keyBlurable `true` if the key press can result in the
	 *   ui_table being blurred. This is so arrow keys won't blur the ui_table, but
	 *   tab will.
	 * @private
	 */
	_shift: function ( e, direction, keyBlurable )
	{
		...
		if ( row >= 0 && row < rows && $.inArray( column, columns ) !== -1 ) {
			...
			// No new focus, but if the ui_table isn't blurable, then don't loose
			// focus
			if (e) {
				e.preventDefault();
			}
	...
	_tabInput: function ()
	{
		...
		// Insert the input element into the first cell in the ui_table's body
...
KeyTable.defaults = {
	/**
	 * Can focus be removed from the ui_table
	 * @type {Boolean}
	 */
	blurable: true,
	...
	/**
	 * Tab index for where the ui_table should sit in the document's tab flow
	 * @type {integer|null}
	 */
	tabIndex: null
};
