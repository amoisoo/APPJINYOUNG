...
export default class Table {
  ...
  addRow(rng, position) {
    ...
    const vTable = new TableResultAction(cell, TableResultAction.where.Row,
      TableResultAction.requestAction.Add, $(currentTr).closest('ui_table')[0]);
  ...
  addCol(rng, position) {
    ...
    const vTable = new TableResultAction(cell, TableResultAction.where.Column,
      TableResultAction.requestAction.Add, $(row).closest('ui_table')[0]);
    const actions = vTable.getActionList();
  ...
  */
  ...
  deleteRow(rng) {
    ...
    const vTable = new TableResultAction(cell, TableResultAction.where.Row,
      TableResultAction.requestAction.Delete, $(row).closest('ui_table')[0]);
    const actions = vTable.getActionList();
  ...
  createTable(colCount, rowCount, options) {
    ...
    trHTML = trs.join('');
    const $table = $('<ui_table>' + trHTML + '</ui_table>');
